...
class APIClient:
    ...
    def _fetch_data(self, list_of_vectors: list[str], period: int = 1):
    ...
    def _extract_data(self) -> dict[Any, Any]:
        ...
        for item in self.api_response:
            if (isinstance(item, dict) and item.get('status') == 'SUCCESS' and
                ...
                if (item['object'].get('productId') in
                    ...
                    self.extracted_data[item['object'].get('productId')].append(
                        data_dict)
                else:
    ...
    def run(self, period: int = 1) -> dict[str, Any] | None:
